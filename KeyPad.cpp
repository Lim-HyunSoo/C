...
4. "left"는 왼손잡이, "right"는 오른손잡이를 의미합니다.
5. 왼손 엄지손가락을 사용한 경우는 L, 오른손 엄지손가락을 사용한 경우는 R을 순서대로 이어붙여 문자열 형태로 return 해주세요.
...
string solution(vector<int> numbers, string hand) {
    ...
    for (int n : numbers){
        if((n == 0) || (n % 3) == 2) {
            ...
            leftD = abs(leftP - n) / 3 + abs(leftP - n) % 3;
            rightD = abs(rightP - n) / 3 + abs(rightP - n) % 3;
            if(leftD < rightD){
                leftP = n;
                answer += "L";
            }
